...
func test_resolve_clip_phase_offsets_accumulates_adjacent_pair_offsets() -> void:
    var phase_offsets := SfxAutomationSyncAnalyzer._resolve_clip_phase_offsets(
        [
            {
    ...
    assert_eq(phase_offsets.size(), 3, "Sequential phase resolution should return one offset per clip")
    assert_almost_eq(phase_offsets[0], 0.0, 0.005, "The first clip should remain the phase anchor")
    assert_almost_eq(phase_offsets[1], 0.1, 0.005, "The second clip should preserve its adjacent lag")
    assert_almost_eq(phase_offsets[2], 0.2, 0.005, "The third clip should accumulate from the previous clip, not re-anchor to the first")
...
func test_select_shared_loop_duration_uses_shortest_clip() -> void:
    ...
    assert_almost_eq(shared_duration, 1.993, 0.001, "Shared loop duration should match the shortest clip, because other loops can only be shortened")
